...
async function initPortal(uid) {
  ...
  $('menuSessions').onclick = () => { setActiveMenu('menuSessions'); showSection('sessionsSection'); loadUpcomingAppointments(uid); }; // Changed function name
  $('menuIncoming').onclick = () => { setActiveMenu('menuIncoming'); showSection('incomingSection'); loadClientRequests(uid); }; // Changed function name
  ...
  $('menuRatings').onclick = () => { setActiveMenu('menuRatings'); showSection('ratingsSection'); loadCounsellorFeedback(uid); }; // Changed function name
  ...
  $('viewIncomingQuick').onclick = () => { setActiveMenu('menuIncoming'); showSection('incomingSection'); loadClientRequests(uid); }; // Changed function name
  ...
  $('sendReportBtn').onclick = async () => { await sendIssueAsCounsellor(uid); }; // Changed function name
  $('clearReportBtn').onclick = () => { $('reportTitle').value = ''; $('reportDesc').value = ''; };

  // Feedback wiring
  $('ratingsFilterBtn').onclick = () => { loadCounsellorFeedback(uid, $('ratingsFilter').value.trim() || ''); }; // Changed function name
...
async function loadDashboard(uid) {
  try {
    ...
    let todaySessions = []; // Appointments
    docs.forEach(s => {
    ...
    $('statThisWeek').textContent = stats.totalThisWeek; // Total Appointments This Week
    $('statUpcoming').textContent = stats.upcoming; // Upcoming Appointments
    $('statPending').textContent = stats.pendingRequests; 
    $('statRating').textContent = stats.avgRating; // Average Feedback Score
...
async function loadUpcomingAppointments(uid) {
  ...
  try {
    ...
    sessions.forEach(s => {
      const dt = s.datetime ? new Date(s.datetime) : null;
      if (!dt || dt < new Date()) return; // Skip past appointments
  ...
  try {
    ...
    requests.forEach(req => {
        ...
        card.querySelector('.approve-req').onclick = async () => {
          ...
          if (conflict) {
              ...
              if (suggested) {
                  ...
                  await updateDoc(doc(db,'sessions',req.id), { suggestedTime: suggested, suggestedBy: 'counsellor', status: 'suggested' }); // Changed tutor to counsellor
                  alert('Suggested time sent to client.'); // Changed student to client
              } else {
        ...
        card.querySelector('.suggest-req').onclick = async () => {
          ...
          await updateDoc(doc(db,'sessions',req.id), { suggestedTime: iso, suggestedBy: 'counsellor', status: 'suggested' }); // Changed tutor to counsellor
          alert('Suggested time sent to client.'); // Changed student to client
          await loadClientRequests(uid);
          await loadPendingCounts(uid);
        };
...
export async function openChatSelector(uid) {
  try {
    ...
    async function loadRecent() {
      ...
      const clients = {}; // Changed students to clients
      sessionsSnap.forEach(d => {
        const s = d.data();
        if (s.studentId) clients[s.studentId] = { id: s.studentId, name: s.studentName, photo: s.studentPhoto }; // Note: retaining studentId/studentName for consistency with the sessions collection schema
      });
    ...
    modal.querySelector('#chatSearchInput').oninput = async (ev) => {
      ...
      const usersSnap = await getDocs(query(collection(db,'users'), where('role','==','student'))); // Assuming clients are stored as role:student
