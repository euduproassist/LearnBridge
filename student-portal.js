...
async function loadTutorDirectory() {
  const listEl = $('tutorDirectory');
  if (!listEl) return;

  listEl.innerHTML = 'Loading tutors...';

  const q = query(collection(db, 'users'), where('role', '==', 'tutor'));
  const snap = await getDocs(q);

  if (snap.empty) {
    listEl.innerHTML = '<p class="empty">No tutors found.</p>';
    return;
  }
  ...
  snap.forEach(doc => {
    ...
    html += `
      <div class="profile-card">
        <img class="profile-photo" src="${u.profilePic || 'https://via.placeholder.com/80'}">
        <h3>${u.name}</h3>
        <p>${u.department || ''}</p>
        <button class="btn" onclick="openBookingModal('tutor',{id:'${doc.id}',name:'${u.name}'})">Book</button>
      </div>
    `;
  });
...
// 2. Load counsellors
async function loadCounsellorDirectory() {
  const listEl = $('counsellorDirectory');
  if (!listEl) return;

  listEl.innerHTML = 'Loading counsellors...';

  const q = query(collection(db, 'users'), where('role', '==', 'counsellor'));
  const snap = await getDocs(q);

  if (snap.empty) {
    listEl.innerHTML = '<p class="empty">No counsellors found.</p>';
    return;
  }
  ...
  snap.forEach(doc => {
    ...
    html += `
      <div class="profile-card">
        <img class="profile-photo" src="${u.profilePic || 'https://via.placeholder.com/80'}">
        <h3>${u.name}</h3>
        <button class="btn" onclick="openBookingModal('counsellor',{id:'${doc.id}',name:'${u.name}'})">Book</button>
      </div>
    `;
  });
...
async function loadUserRatingsCount(uid) {
  ...
  const snap = await getDocs(q);
  $('ratingsCount').textContent = snap.size;
}

// 4. Ratings list
async function loadRatingsList(uid) {
  const listEl = $('ratingsList');
...
async function loadSessionSummaries(uid) {
  ...
  snap.forEach(doc => {
    const s = doc.data();
    const future = new Date(s.datetime) > new Date();
    if (!future) return;

    if (s.role === 'tutor') tcount++;
    if (s.role === 'counsellor') ccount++;
  });

  $('tutorSessionSummary').textContent =
    tcount > 0 ? `${tcount} upcoming session(s)` : 'No upcoming tutor sessions.';

  $('counsellorSessionSummary').textContent =
    ccount > 0 ? `${ccount} upcoming session(s)` : 'No upcoming counsellor sessions.';
}
