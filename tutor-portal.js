...
/* ---------- STUDENT REQUESTS (Pending requests) - for tutors to act on (was loadClientRequests) ---------- */
async function loadStudentRequests(uid) {
  ...
  try {
    ...
    if (requests.length === 0) {
      container.innerHTML = '';
      show('incomingEmpty');
      emptyEl.textContent = 'No incoming student requests.';
      return;
    }
    hide('incomingEmpty');
        ...
        card.innerHTML = `
            ...
            <div style="flex:1">
                ...
                <div>
                    ...
                    <div class="muted">${escapeHtml(req.module || req.course || '')} (Module)</div>
                </div>
    ...
    container.innerHTML = `<div class="empty">Failed to load incoming student requests</div>`;
  }
}
