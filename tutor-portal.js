...
async function handleCompleteTutorial(sessionId) {
  ...
  try {
    ...
    alert(`Tutorial marked complete. Duration: ${durationMins} minutes.`);
...
async function loadLessonHistory(uid) {
  const container = $('historyList'); // Ensure you add this ID to your HTML
  if (!container) return;
  container.innerHTML = 'Loading lesson history...';
  ...
  try {
    ...
    if (snap.empty) {
      container.innerHTML = '<div class="empty">No completed lessons yet.</div>';
      return;
    }
    ...
    container.innerHTML = snap.docs.map(doc => {
      ...
      return `
        <div class="profile-card" style="margin-bottom:15px; border-left: 5px solid #006064;">
          ...
          <div style="margin-top:10px; padding:8px; background:#f9f9f9; border-radius:4px; font-size:12px;">
            <strong>Student Note:</strong> "${escapeHtml(s.notes || 'No description provided')}"
          </div>
        </div>
      `;
    }).join('');
    ...
    container.innerHTML = '<div class="empty">Error loading history.</div>';
  }
}
