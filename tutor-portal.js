...
async function initPortal(uid) {
  ...
  $('menuSessions').onclick = () => { setActiveMenu('menuSessions'); showSection('sessionsSection'); loadUpcomingTutorials(uid); }; // Renamed function
  $('menuIncoming').onclick = () => { setActiveMenu('menuIncoming'); showSection('incomingSection'); loadStudentRequests(uid); }; // Renamed function
  ...
  $('menuRatings').onclick = () => { setActiveMenu('menuRatings'); showSection('ratingsSection'); loadTutorFeedback(uid); }; // Renamed function
  ...
  $('viewIncomingQuick').onclick = () => { setActiveMenu('menuIncoming'); showSection('incomingSection'); loadStudentRequests(uid); }; // Renamed function
  ...
  $('sendReportBtn').onclick = async () => { await sendIssueAsTutor(uid); }; // Renamed function
  $('clearReportBtn').onclick = () => { $('reportTitle').value = ''; $('reportDesc').value = ''; };

  // Feedback wiring
  $('ratingsFilterBtn').onclick = () => { loadTutorFeedback(uid, $('ratingsFilter').value.trim() || ''); }; // Renamed function
...
/* ---------- Profile for Tutor (UPDATED Fields) ---------- */
async function loadProfile(uid) {
  try {
    ...
    if (profile) {
      ...
      $('profileBioInput').value = profile.bio || ''; // Subjects/Expertise
      $('profileModulesInput').value = profile.modules || ''; // Subjects Tutored
      $('profileDepartmentInput').value = profile.department || '';
      $('profileQualificationsInput').value = profile.qualifications || ''; // Qualifications/Skills
      $('profilePictureInput').value = profile.profilePictureInput || '';
      $('profileLocationInput').value = profile.location || ''; // Location (Campus & Online)
      $('profileRateInput').value = profile.rate || ''; // Tutorial Rate
    } else {
...
async function saveProfile(uid) {
  try {
    const payload = {
      ...
      role: STATE.profile?.role || 'tutor' // Changed default role to tutor
    };
...
async function loadDashboard(uid) {
  try {
    ...
    $('statThisWeek').textContent = stats.totalThisWeek; // Total Tutorials This Week
    $('statUpcoming').textContent = stats.upcoming; // Upcoming Tutorials
    $('statPending').textContent = stats.pendingRequests; 
    $('statRating').textContent = stats.avgRating; // Average Review Score
    
    // Today's schedule
    if (todaySessions.length) {
        $('todayList').innerHTML = todaySessions.slice(0, 3).map(s => 
          `<p style="margin-bottom:4px;">${new Date(s.datetime).toLocaleTimeString()} - ${s.studentName}</p>` // Student name
        ).join('');
        if (todaySessions.length > 3) {
            $('todayList').innerHTML += `<small class="muted">...and ${todaySessions.length - 3} more.</small>`;
        }
    } else {
        $('todayList').textContent = "No tutorials today."; // Changed appointments to tutorials
    }
    ...
    stats.notificationsSummary = notifSnap.size > 0 ? `${notifSnap.size} tutorial actions pending.` : 'All clear.'; // Changed appointment to tutorial
...
/* ---------- Tutorials List (renamed from loadUpcomingAppointments) ---------- */
async function loadUpcomingTutorials(uid) {
  ...
  try {
    ...
    if (sessions.length === 0) {
      container.innerHTML = '<div class="empty">No upcoming tutorials.</div>'; // Changed appointments to tutorials
      return;
    }

    container.innerHTML = '<table><thead><tr><th>Time</th><th>Student</th><th>Subject</th><th>Mode</th><th>Actions</th></tr></thead><tbody>'; // Changed Client to Student, Service to Subject
    sessions.forEach(s => {
      const dt = s.datetime ? new Date(s.datetime) : null;
      if (!dt || dt < new Date()) return; // Skip past tutorials
    ...
    // Handlers
    container.querySelectorAll('.start-session').forEach(btn => btn.onclick = (e) => handleStartTutorial(e.target.dataset.id)); // Renamed handler
    // container.querySelectorAll('.reschedule-session').forEach(btn => btn.onclick = (e) => openRescheduleRequestModal(e.target.dataset.id)); // Assuming modal logic is deferred
  
  } catch (err) {
    console.error('loadUpcomingTutorials', err);
    container.innerHTML = '<div class="empty">Failed to load tutorials.</div>'; // Changed appointments to tutorials
  }
}

/* ---------- Action: Start tutorial (UPDATED terminology) ---------- */
async function handleStartTutorial(sessionId) {
  try {
    const sRef = doc(db, 'sessions', sessionId);
    const snap = await getDoc(sRef);
    if (!snap.exists()) return alert('Tutorial not found'); // Changed appointment to tutorial
    const s = snap.data();
    
    // check mode online and time window
    if (s.mode !== 'online') {
      return alert('This tutorial is not online. Start in-person tutorials at the scheduled location.'); // Changed appointment to tutorial
    }
    // Set status to in-progress
    await updateDoc(sRef, { status: 'in-progress', startedAt: new Date().toISOString() });
    alert('Tutorial started. You can now chat / share materials.'); // Changed appointment to tutorial
    ...
    alert('Failed to start tutorial: ' + err.message); // Changed appointment to tutorial
  }
}

/* ---------- STUDENT REQUESTS (Pending requests) - for tutors to act on (UPDATED terminology) ---------- */
async function loadStudentRequests(uid) {
  const container = $('incomingList'); 
  const emptyEl = $('incomingEmpty'); 
  if (!container) return;
  container.innerHTML = 'Loading incoming student requests...'; // Changed client to student
  hide('incomingEmpty');
  ...
  try {
    ...
    if (requests.length === 0) {
      container.innerHTML = '';
      show('incomingEmpty');
      emptyEl.textContent = 'No incoming student requests.'; // Changed client to student
      return;
    }
    hide('incomingEmpty');
    ...
    requests.forEach(req => {
        ...
        let statusBadge = req.status === 'suggested' ? `<span class="badge" style="background:orange">Awaiting Student Confirmation</span>` : `<span class="badge">Pending Your Approval</span>`; // Changed client to student
        ...
        card.innerHTML = `
            ...
            <div style="flex:1">
                ...
                <div>
                    ...
                    <div class="muted">${escapeHtml(req.module || req.course || '')} (Subject)</div>
                </div>
        ...
        card.querySelector('.approve-req').onclick = async () => {
          ...
          if (conflict) {
              ...
              if (suggested) {
                  ...
                  await updateDoc(doc(db,'sessions',req.id), { suggestedTime: suggested, suggestedBy: 'tutor', status: 'suggested' }); // Changed counsellor to tutor
                  alert('Suggested time sent to student.'); // Changed client to student
              } else {
          ...
          await loadStudentRequests(uid); // Renamed function
          await loadPendingCounts(uid);
        };
        ...
        card.querySelector('.suggest-req').onclick = async () => {
          ...
          await updateDoc(doc(db,'sessions',req.id), { suggestedTime: iso, suggestedBy: 'tutor', status: 'suggested' }); // Changed counsellor to tutor
          alert('Suggested time sent to student.'); // Changed client to student
          await loadStudentRequests(uid); // Renamed function
          await loadPendingCounts(uid);
        };
    ...
    container.innerHTML = `<div class="empty">Failed to load incoming student requests</div>`; // Changed client to student
  }
}
  ...
  try {
    ...
    modal.innerHTML = `
      <div class="modal">
        <h3>Start Chat</h3>
        <div>
          <input id="chatSearchInput" placeholder="Search student name or subject" style="width:100%;margin-bottom:8px" />
    ...
    async function loadRecent() {
      results.innerHTML = 'Loading recent students...'; // Changed clients to students
      const sessionsSnap = await getDocs(query(collection(db,'sessions'), where('personId','==', uid), orderBy('createdAt','desc'), limit(50)));
      const clients = {}; // Retaining variable name clients for internal map
      sessionsSnap.forEach(d => {
        const s = d.data();
        if (s.studentId) clients[s.studentId] = { id: s.studentId, name: s.studentName, photo: s.studentPhoto }; 
      });
      const list = Object.values(clients);
      if (list.length === 0) results.innerHTML = '<div class="empty">No recent students. Try searching.</div>'; // Changed clients to students
      else {
    ...
    modal.querySelector('#chatSearchInput').oninput = async (ev) => {
      ...
      const usersSnap = await getDocs(query(collection(db,'users'), where('role','==','student'))); // Retaining 'student' role for clients
      ...
      if (users.length === 0) results.innerHTML = '<div class="empty">No matches</div>';
        ...
        results.innerHTML = users.map(u => `
          <div style="display:flex;gap:10px;align-items:center;padding:8px;border-bottom:1px solid #eee">
            ...
            <div style="flex:1"><strong>${escapeHtml(u.name||'Student')}</strong><div class="muted">${escapeHtml(u.modules||'Subject')}</div></div>
...
/* Cleanup helper: remove all chat listeners (remains same) */
...
/* ---------- Student Feedback (renamed from loadCounsellorFeedback) ---------- */
async function loadTutorFeedback(uid, filter = '') {
  ...
  try {
    ...
    if (filter) {
        // Simple text filter for notes/service name (assuming 'module' is subject name in session)
    ...
    if (feedback.length === 0) {
      container.innerHTML = '';
      show('ratingsEmpty');
      emptyEl.textContent = 'No student feedback yet.'; // Changed client to student
      return;
    }
    hide('ratingsEmpty');

    container.innerHTML = feedback.map(f => {
        const stars = f.stars || '—'; // Used for star rating (can be repurposed for score/urgency level)
        const urgencyClass = (f.urgency === 'Urgent') ? 'urgent' : '';
        const clientName = f.studentName || 'Anonymous Student'; // Changed Client to Student
        ...
        return `
          <div class="profile-card ${urgencyClass}" style="margin-bottom:12px">
            ...
            <p class="muted" style="margin-top:6px">Subject: ${escapeHtml(f.service || 'N/A')}</p>
...
/* ---------- Report an Issue (Tutor - UPDATED role) ---------- */
async function sendIssueAsTutor(uid) {
  try {
    ...
    await addDoc(collection(db,'issues'), {
      ...
      category,
      status: 'open',
      role: 'tutor', // Changed role to tutor
